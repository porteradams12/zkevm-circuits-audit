...
use boojum::gadgets::traits::allocatable::{CSAllocatableExt, CSAllocatable, CSPlaceholder};
...
fn u64_as_u32x2_conditionally_increment<F: SmallField, CS: ConstraintSystem<F>>(
    ...
    let selected = Selectable::conditionally_select(cs, *should_increment, &[incremented_low, incremented_high], input);
...
pub(crate) fn keccak256_conditionally_absorb_and_run_permutation<F: SmallField, CS: ConstraintSystem<F>>(
    ...
    for i in 0..keccak256::LANE_WIDTH {
        for j in 0..keccak256::LANE_WIDTH {
            if i + keccak256::LANE_WIDTH * j < (keccak256::KECCAK_RATE_BYTES / keccak256::BYTES_PER_WORD) {
                let tmp = block.array_chunks::<{keccak256::BYTES_PER_WORD}>().skip(i + keccak256::LANE_WIDTH * j).next().unwrap();
...
impl <
    F: SmallField,
    EL: CSAllocatableExt<F>,
> ConditionalWitnessAllocator<F, EL> where [(); EL::INTERNAL_STRUCT_LEN]:, [(); EL::INTERNAL_STRUCT_LEN + 1]: {
    pub fn conditionally_allocate_with_default<CS: ConstraintSystem<F>, DEF: FnOnce() -> EL::Witness + 'static + Send + Sync>(
        &self,
        cs: &mut CS,
    ...
    pub fn conditionally_allocate<CS: ConstraintSystem<F>>(
        ...
        cs: &mut CS,
        should_allocate: Boolean<F>,
    ) -> EL where EL::Witness: Default {
        self.conditionally_allocate_with_default(
            cs,
            should_allocate,
...
where [(); <LogQuery<F> as CSAllocatableExt<F>>::INTERNAL_STRUCT_LEN]: {
    ...
    let leaf_indexes_for_reads: VecDeque<_> = leaf_indexes_for_reads.into_iter().map(|el| {
    ...
    let storage_queue_state_from_input = 
        structured_input
            .observable_input
    ...
    let storage_queue_state_from_fsm = 
        structured_input
            .hidden_fsm_input
    ...
    let mut storage_accesses_queue = StorageLogQueue::<F, R>::from_state(
        cs,
        storage_accesses_queue_state
    );
    ...
    let mut diffs_keccak_accumulator_state = [[[zero_u8; keccak256::BYTES_PER_WORD]; keccak256::LANE_WIDTH]; keccak256::LANE_WIDTH];
    let keccak_sponge_state_from_fsm = structured_input
        .hidden_fsm_input
        .current_diffs_keccak_accumulator_state;

    for (a, b) in diffs_keccak_accumulator_state.iter_mut().zip(keccak_sponge_state_from_fsm.iter()) {
        for (a, b) in a.iter_mut().zip(b.iter()) {
            *a = UInt8::parallel_select(cs, start_flag, &*a, b);
        }
    }

    let mut diffs_keccak_accumulator_state = diffs_keccak_accumulator_state.map(|el| {
        el.map(|el| {
            el.map(|el| el.get_variable())
        })
    });
    ...
    for cycle in 0..limit {
        ...
        let (should_compare_roots, parse_next_queue_elem) = if is_last == false {
            ...
            let parse_next_queue_elem =
                should_compare_roots.and(cs, read_stage_in_progress);
        ...
        let (storage_log, _) =
            storage_accesses_queue.pop_front(cs, parse_next_queue_elem);
        ...
        let aux_byte_is_valid = UInt8::equals(
            cs,
            &storage_log.aux_byte,
            &storage_aux_byte,
        );
        ...
        let read_index = allocate_enumeration_index_from_witness(cs, parse_next_queue_elem, read_index_witness_allocator.clone());
        ...
        let should_assign_fresh_idx = Boolean::multi_and(cs, &[write_stage_in_progress, current_idx_is_zero]);
        ...
        current_next_enumeration_index = u64_as_u32x2_conditionally_increment(cs, &current_next_enumeration_index, &should_assign_fresh_idx);

        // index is done, now we need merkle path
        let new_merkle_path_witness = allocate_merkle_path_from_witness(cs, parse_next_queue_elem, merkle_path_witness_allocator.clone());
        // if we read then we save and use it for write too
        for (dst, src) in merkle_path_witness.iter_mut().zip(new_merkle_path_witness.into_iter()) {
        ...
        leaf_value_for_this_stage = UInt8::parallel_select(cs, write_stage_in_progress, &saved_written_value, &leaf_value_for_this_stage);
        ...
        for (path_bit, path_witness) in path_selectors.into_iter().zip(merkle_path_witness.into_iter()) {
        ...
        // update if we write
        current_root_hash = UInt8::parallel_select(cs, write_stage_in_progress, &current_hash, &current_root_hash);
        ...
        let mut extended_state_diff_encoding = [zero_u8; keccak256::KECCAK_RATE_BYTES * NUM_KECCAK256_ROUNDS_PER_RECORD_ACCUMULATION];
        ...
        if is_first == false {
            for block in extended_state_diff_encoding.array_chunks::<{keccak256::KECCAK_RATE_BYTES}>() {
        ...
        let cur_elem_was_processed = Boolean::multi_or(cs, &[write_stage_in_progress, current_element_is_read]);
    ...
    let current_diffs_keccak_accumulator_state_for_fsm = unsafe {
        diffs_keccak_accumulator_state.map(|el| {
            el.map(|el| {
                el.map(|el| UInt8::from_variable_unchecked(el))
            })
        })
    };
    ...
    for (i, dst) in result.array_chunks_mut::<8>().enumerate() {
        for (dst, src) in dst.iter_mut().zip(diffs_keccak_accumulator_state[i][0].iter()) {
