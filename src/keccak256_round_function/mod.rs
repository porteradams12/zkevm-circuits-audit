...
pub const BUFFER_SIZE_IN_U64_WORDS: usize = MEMORY_EQURIES_PER_CYCLE * 4 + KECCAK256_RATE_IN_U64_WORDS - 1;
    ...
    [(); <UInt256<F> as CSAllocatableExt<F>>::INTERNAL_STRUCT_LEN + 1]: 
{
    ...
    let buffer_len_bound = UInt16::allocated_constant(cs, (BUFFER_SIZE_IN_U64_WORDS - NUM_U64_WORDS_PER_CYCLE + 1) as u16);
    ...
    for _cycle in 0..limit {
        ...
        let initial_buffer_len = {
            let lc: Vec<_> = state.u64_words_buffer_markers.iter().map(|el| (el.get_variable(), F::ONE)).collect();
        ...
        state.read_unaligned_words_for_round.conditionally_enforce_true(cs, can_not_fill);
        ...
        tmp_buffer[..(BYTES_BUFFER_SIZE - keccak256::KECCAK_RATE_BYTES)].copy_from_slice(&state.u8_words_buffer[keccak256::KECCAK_RATE_BYTES..]);

        // also reset markers
        let mut tmp_buffer_markers = [boolean_false; BUFFER_SIZE_IN_U64_WORDS];
        tmp_buffer_markers[..(BUFFER_SIZE_IN_U64_WORDS - KECCAK256_RATE_IN_U64_WORDS)].copy_from_slice(&state.u64_words_buffer_markers[KECCAK256_RATE_IN_U64_WORDS..]);
        ...
        let squeezed = keccak256_absorb_and_run_permutation(
            cs,
            &mut state.keccak_internal_state,
            &input
        );

        let no_rounds_left = state.precompile_call_params.num_rounds.is_zero(cs);
        let write_result = Boolean::multi_and(cs, &[state.read_unaligned_words_for_round, no_rounds_left]);
...
pub(crate) fn keccak256_absorb_and_run_permutation<F: SmallField, CS: ConstraintSystem<F>>(
    cs: &mut CS,
    state: &mut [[[UInt8<F>; keccak256::BYTES_PER_WORD]; keccak256::LANE_WIDTH]; keccak256::LANE_WIDTH],
    block: &[UInt8<F>; keccak256::KECCAK_RATE_BYTES],
) -> [UInt8<F>; keccak256::KECCAK256_DIGEST_SIZE] {
    let mut state_as_variables = state.map(|el| {
        el.map(|el| {
    ...
    *state = new_state;
    ...
    // copy back
    let mut result = [std::mem::MaybeUninit::<UInt8<F>>::uninit(); keccak256::KECCAK256_DIGEST_SIZE];
