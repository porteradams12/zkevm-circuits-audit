...
>(
    ...
    const SHL_OPCODE: zkevm_opcode_defs::Opcode = zkevm_opcode_defs::Opcode::Shift(zkevm_opcode_defs::definitions::shift::ShiftOpcode::Shl);
    const ROL_OPCODE: zkevm_opcode_defs::Opcode = zkevm_opcode_defs::Opcode::Shift(zkevm_opcode_defs::definitions::shift::ShiftOpcode::Rol);
    const SHR_OPCODE: zkevm_opcode_defs::Opcode = zkevm_opcode_defs::Opcode::Shift(zkevm_opcode_defs::definitions::shift::ShiftOpcode::Shr);
    const ROR_OPCODE: zkevm_opcode_defs::Opcode = zkevm_opcode_defs::Opcode::Shift(zkevm_opcode_defs::definitions::shift::ShiftOpcode::Ror);

    let should_apply = common_opcode_state.decoded_opcode.properties_bits.boolean_for_opcode(SHL_OPCODE);
    ...
    let should_set_flags = common_opcode_state.decoded_opcode.properties_bits.flag_booleans[SET_FLAGS_FLAG_IDX];

    let is_rol = common_opcode_state.decoded_opcode.properties_bits.boolean_for_variant(ROL_OPCODE);
    let is_ror = common_opcode_state.decoded_opcode.properties_bits.boolean_for_variant(ROR_OPCODE);
    let is_shr = common_opcode_state.decoded_opcode.properties_bits.boolean_for_variant(SHR_OPCODE);
    ...
    let inverted_shift = unsafe {
        UInt8::from_variable_unchecked(cnst.get_variable()).sub_no_overflow(cs, shift)
    };
    ...
    let full_shift_limbs = get_shift_constant(
        cs,
        full_shift
    );
    ...
    let (rshift_q, _rshift_r) = allocate_div_result_unchecked(
        cs, &reg, &full_shift_limbs
    );
    ...
    let (lshift_low, lshift_high) = allocate_mul_result_unchecked(
        cs, &reg, &full_shift_limbs
    );
    ...
    let rem_to_enforce = UInt32::parallel_select(
        cs, apply_left_shift, &uint256_zero.inner, &_rshift_r,
    );
    let a_to_enforce = UInt32::parallel_select(
        cs, apply_left_shift, reg, &rshift_q,
    );
    let b_to_enforce = full_shift_limbs;
    let mul_low_to_enforce = UInt32::parallel_select(
        cs, apply_left_shift, &lshift_low, reg,
    );
    let mul_high_to_enforce = UInt32::parallel_select(
        cs, apply_left_shift, &lshift_high, &uint256_zero.inner,
    );
    ...
    for (limb_in, of_in, limb_out) in itertools::multizip(zipped_iter) {
        ...
        let res = Num::fma(
            cs, &of_in.into_num(), &is_cyclic.into_num(), &F::ONE, &limb_in.into_num(), &F::ONE
        );
    ...
    let dst0 = VMRegister {
        ...
        value: UInt256 {inner: final_result}
    };

    let can_write_into_memory = SHL_OPCODE.can_write_dst0_into_memory(SUPPORTED_ISA_VERSION);

    diffs_accumulator.dst_0_values.push((can_write_into_memory, should_apply, dst0));
    diffs_accumulator.flags.push((set_flags_and_execute, new_flag_port));

    let mut mul_div_relations = ArrayVec::new();
    mul_div_relations.push(mul_relation);
    diffs_accumulator.mul_div_relations.push((should_apply, mul_div_relations));
}
...
pub(crate) fn get_shift_constant<F: SmallField, CS: ConstraintSystem<F>>(
    ...
    let shift_table_id = cs.get_table_id_for_marker::<BitshiftTable>().expect("table must exist");
