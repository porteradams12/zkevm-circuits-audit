...
pub fn allocate_mul_result_unchecked<F: SmallField, CS: ConstraintSystem<F>>(
    ...
    if <CS::Config as CSConfig>::WitnessConfig::EVALUATE_WITNESS {
        ...
        let dependencies= Place::from_variables([
            a[0].get_variable(), a[1].get_variable(), a[2].get_variable(), a[3].get_variable(), 
            a[4].get_variable(), a[5].get_variable(), a[6].get_variable(), a[7].get_variable(),
            b[0].get_variable(), b[1].get_variable(), b[2].get_variable(), b[3].get_variable(), 
            b[4].get_variable(), b[5].get_variable(), b[6].get_variable(), b[7].get_variable(),
        ]);
        let outputs = Place::from_variables([
            limbs_low[0], limbs_low[1], limbs_low[2], limbs_low[3], 
            limbs_low[4], limbs_low[5], limbs_low[6], limbs_low[7], 
            limbs_high[0], limbs_high[1], limbs_high[2], limbs_high[3], 
            limbs_high[4], limbs_high[5], limbs_high[6], limbs_high[7],
        ]);
...
pub fn all_limbs_are_equal<F: SmallField, CS: ConstraintSystem<F>>(
    cs: &mut CS, lhs: &[UInt32<F>; 8], rhs: &[UInt32<F>; 8]
) -> Boolean<F> {
...
>(
    ...
    const MUL_OPCODE: zkevm_opcode_defs::Opcode = zkevm_opcode_defs::Opcode::Mul(zkevm_opcode_defs::MulOpcode);
    const DIV_OPCODE: zkevm_opcode_defs::Opcode = zkevm_opcode_defs::Opcode::Div(zkevm_opcode_defs::DivOpcode);

    let should_apply_mul = common_opcode_state.decoded_opcode.properties_bits.boolean_for_opcode(MUL_OPCODE);
    let should_apply_div = common_opcode_state.decoded_opcode.properties_bits.boolean_for_opcode(DIV_OPCODE);
    ...
    let to_enforce_1 = UInt32::parallel_select(
        cs, should_apply_mul, &mul_high_unchecked, &remainder_unchecked
    );
    ...
    let mul_low_to_enforce = UInt32::parallel_select(
        cs, should_apply_mul, &mul_low_unchecked, &src0_view,
    );
    let mul_high_to_enforce = UInt32::parallel_select(
        cs, should_apply_mul, &mul_high_unchecked, &uint256_zero.inner,
    );
    ...
    diffs_accumulator.dst_0_values.push((can_write_into_memory, apply_any, dst0));
    ...
    diffs_accumulator.flags.push((set_flags_and_execute, candidate_flags));
