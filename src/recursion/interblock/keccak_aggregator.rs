use boojum::gadgets::{u8::UInt8, traits::selectable::Selectable};

use super::*;

pub struct KeccakPublicInputAggregator<F: SmallField, const N: usize, const IS_BE: bool, const NUM_OUTS: usize>{
    pub masking_value: u8,
    _marker: std::marker::PhantomData<F>
}

impl<F: SmallField, const N: usize, const IS_BE: bool, const NUM_OUTS: usize> InputAggregationFunction<F> for KeccakPublicInputAggregator<F, N, IS_BE, NUM_OUTS> {
    type Params = u8;
    ...
    fn aggregate_inputs<CS: ConstraintSystem<F>>(&self, cs: &mut CS, inputs: &[Vec<Num<F>>], validity_flags: &[Boolean<F>]) -> Vec<Num<F>> {
        ...
        for (validity_flag, input) in validity_flags.iter().zip(inputs.iter()) {
            ...
            for src in input.iter() {
                let mut bytes: arrayvec::ArrayVec<UInt8<F>, 8> = src.constraint_bit_length_as_bytes(cs, total_byte_len); // le
                if F::CAPACITY_BITS % 8 != 0 {
        ...
        for chunk in aggregated_keccak_hash
